...
public class Settings implements Screen {
	...
	public static FileHandle levelDirectory() {
		...
		if(prefsDir != null && !prefsDir.equals(""))
			...
			return Gdx.files.absolute(Gdx.files.external(landthat.TITLE + "/levels").path()); // return default level directory
	}

	/** @return if vSync is enabled */
	public static boolean vSync() {
		return Gdx.app.getPreferences(landthat.TITLE).getBoolean("vsync");
	}
		...
		ClickListener buttonHandler = new ClickListener() {
			...
			public void clicked(InputEvent event, float x, float y) {
				...
				if(event.getListenerActor() == vSyncCheckBox) {
					// save vSync
					Gdx.app.getPreferences(landthat.TITLE).putBoolean("vsync", vSyncCheckBox.isChecked());

					// set vSync
					Gdx.graphics.setVSync(vSync());

					Gdx.app.log(landthat.TITLE, "vSync " + (vSync() ? "enabled" : "disabled"));
				} else if(event.getListenerActor() == back) {
					// save level directory
					String actualLevelDirectory = levelDirectoryInput.getText().trim().equals("") ? Gdx.files.getExternalStoragePath() + landthat.TITLE + "/levels" : levelDirectoryInput.getText().trim(); // shortened form of an if-statement: [boolean] ? [if true] : [else] // String#trim() removes spaces on both sides of the string
					Gdx.app.getPreferences(landthat.TITLE).putString("leveldirectory", actualLevelDirectory);

					// save the settings to preferences file (Preferences#flush() writes the preferences in memory to the file)
					Gdx.app.getPreferences(landthat.TITLE).flush();

					Gdx.app.log(landthat.TITLE, "settings saved");
